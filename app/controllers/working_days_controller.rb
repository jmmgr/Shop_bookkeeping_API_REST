class WorkingDaysController < ApplicationController
  ...
  def index
    @working_days = WorkingDay.all.order(:day_date)
  end
  
  def by_date
   #  WorkingDay.where("day_date = \'"+params[:day_date]+"\'")
    @working_day_by_date = WorkingDay.where(:day_date=> params[:day_date])  
    #@working_day_by_date = WorkingDay.where (:day_date => params[:day_date])    
      
    #("day_date = \'"+params[:day_date]+"\' limit(1)")
  end
  # GET /working_days/1
  ...
  private
    ...
    def working_day_params
      params.require(:working_day).permit(:day_date, :sale, :cash, :change_coins)
    end
end
